...
const getSquadHeaders = () => {
  ...
  if (env === 'sandbox' && !apiKey.startsWith('sandbox_sk_')) {
    throw new Error('SQUAD_SECRET_KEY must start with "sandbox_sk_" when environment is sandbox.');
  }
  
  if (env === 'production' && !apiKey.startsWith('sk_')) {
    throw new Error('SQUAD_SECRET_KEY must start with "sk_" when environment is production.');
  }
